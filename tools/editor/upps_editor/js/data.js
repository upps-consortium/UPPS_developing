...
function handleEmotionModelChange() {
    const model = this.persona?.emotion_system?.model || 'Ekman';
...
function handleCognitiveModelChange() {
    const model = this.persona?.cognitive_system?.model || 'WAIS-IV';
...
function syncEmotionState() {
    if (!this.persona || !this.persona.emotion_system || !this.persona.emotion_system.emotions) {
        console.warn('Cannot sync emotion state: emotion system not initialized');
        return;
    }
    ...
    this.networkVisualization.nodes.forEach(node => {
        if (node.type === 'emotion' && this.persona.current_emotion_state[node.emotionId]) {
            ...
            try {
                ...
                console.warn('Error updating visualizer nodes:', error);
            }
        }
    });
}
...
// プロトタイプに関数を割り当て
window.getDefaultPersona = getDefaultPersona;
UPPSEditor.prototype.initializePersona = initializePersona;
UPPSEditor.prototype.handleEmotionModelChange = handleEmotionModelChange;
UPPSEditor.prototype.handleCognitiveModelChange = handleCognitiveModelChange;
UPPSEditor.prototype.syncEmotionState = syncEmotionState;
UPPSEditor.prototype.updateVisualizerFromEmotionState = updateVisualizerFromEmotionState;
UPPSEditor.prototype.updateCognitiveAbilityDisplay = updateCognitiveAbilityDisplay;
UPPSEditor.prototype.updateCognitiveAbility = updateCognitiveAbility;
UPPSEditor.prototype.loadFromLocalStorage = loadFromLocalStorage;
