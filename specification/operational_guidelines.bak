...
> *「Unified Personality Profile Standard の効果的な活用と管理のためのガイドライン」*
...
- 人格情報には、性格特性（Personality）、背景情報（Background）、感情記憶（Memory_Trace）、セッション文脈（Session_Context）、内部状態（State）が含まれる
...
- Stateは感情名（例：calm, anxious, tired）と数値（0〜100）で表現する
- 対話中、Stateは動的に変化する
